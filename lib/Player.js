...
class Player {
    ...
    update = async device => {
        try {
            ...
            logger.error(`Cant update because: ${err.message}`, {
    ...
    setVolume = volume => {
        if (this.device !== null) {
            this.device.setVolume(volume / 100, err => {
                if (error) {
                    logger.error(
                        `couldnt set volume on device ${this.device.friendlyName}`,
                        {
                            error: makeError(error),
                            ...tag.CAST
                        }
    ...
    devicePause = () => {
        if (this.device !== null) {
            this.device.pause(err => {
                if (error) {
                    logger.error(
                        `couldnt pause on device ${this.device.friendlyName}`,
                        {
                            error: makeError(error),
                            ...tag.CAST
                        }
    ...
    deviceSeek = time => {
        if (this.device !== null) {
            this.device.seek(time, err => {
                if (error) {
                    logger.error(
                        `couldnt seek on device ${this.device.friendlyName}`,
                        {
                            error: makeError(error),
                            ...tag.CAST
                        }
    ...
    deviceResume = () => {
        if (this.device !== null) {
            this.device.resume(err => {
                if (error) {
                    logger.error(
                        `couldnt resume on device ${this.device.friendlyName}`,
                        {
                            error: makeError(error),
                            ...tag.CAST
                        }
        ...
        if (this.device !== null) {
            this.device.play(
                ...
                error => {
                    if (error) {
                        logger.error(
                            `couldnt play song ${song.title} on device ${this.device.friendlyName}`,
                            {
                                error: makeError(error),
                                ...tag.CAST
                            }
    ...
    updatePlaylist = playlist => {
        this.window.webContents.send(`playlist`, playlist);
        if (playlist !== null) {
            this.notify(`Current Playlist: ${playlist.name}`);
        }
    };
            ...
            try {
                this.device.close(() => {
                    ...
                    try {
                        ...
                        logger.error(`couldnt send device status`, {
                            error: makeError(error),
                            ...tag.PLAYER
                        });
                    }
