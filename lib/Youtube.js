...
const getAudioUrl = async (url) => {
    ...
    formatWithBestAudioQlty = formats.filter(format => format.mimeType.startsWith('audio')).reduce((prev, current) => {
        return prev.averageBitrate > current.averageBitrate ? prev : current;
    });
    if (formatWithBestAudioQlty) {
        const { dec } = require(decoderPath);
        let audioUrl = formatWithBestAudioQlty.cipher.url + '&' + formatWithBestAudioQlty.cipher.sp + '=' + dec(formatWithBestAudioQlty.cipher.s);
        return audioUrl;
    } else {
