        ...
        searchWindow.webContents.session.webRequest.onBeforeSendHeaders(
            (details, callback) => {
                ...
                if (details.url.indexOf("mime=audio") !== -1) {
                    song.audioUrl = removeURLParameters(details.url, [
                        "range",
                        "rn",
                        "rbuf"
                    ]);
                    ...
                    song.expiresOn = parseInt(
                        getURLParameter(details.url, "expire")
                    );

                    axios
                        .head(song.audioUrl + '2323')
                        .then(res => {
                            ...
                            if (res.status == 200)
                                ...
                                reject(new Error(`Got status ${res.status} - ${res.statusText}`));
                        })
                        .catch(error => {
                            clearTimeout(timerId);
                            reject(error);
