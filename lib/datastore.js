...
class DataStore extends Store {
    ...
    errorIfExists(tracks, songId) {
        let result = tracks.filter(it => it.id === songId);
        if (result.length > 0)
            throw new Error(`Song already exists`);
    }

    addSong(playlistId, song, from) {
        song.addedBy = from;
        let playlist = this.getPlaylist(playlistId);
        if (playlist !== null) {
            this.errorIfExists(playlist.tracks, song.id);
    ...
    putSongNext(playlistId, song, from) {
        song.addedBy = from;
        let playlist = this.getPlaylist(playlistId);
